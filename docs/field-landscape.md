...
### 5. 互动而非复现（Shimon 与 Dan Tepfer 的路线）

**Dan Tepfer《Natural Machines》（2019）是这条线上最直接的钢琴案例**——受过天体物理训练的
爵士钢琴家，自学编程，**给 Yamaha Disklavier 写自己的代码**：他即兴演奏，程序实时读取他弹的音，
再指挥钢琴弹出应答（"Canon at the Octave" 用的算法极简——把同一个音升高八度返回），
同时把数据渲染成实时可视化。他自己的说法是：
> "我与其说在写一首曲子，不如说在写这首曲子的运作方式。"

**注意他做了什么、没做什么：**

| | |
|---|---|
| 他做的 | 实时算法作曲、人机音乐互动、可视化——**全部是软件与音乐** |
| 他没做的 | **任何机械**。他买了一台现成的 Disklavier |

他的突破点是发现"可以把电脑的实时输入喂进这台琴"——按他的说法，**这并非该乐器的预期用途**。
这反过来对本仓库的项目是个正面证据：这条链路是通的。

**但要记住 Disklavier 的接口形状**：它接受的是一个**标量**（目标击弦速度），
内部自行决定如何驱动电磁铁。你无法命令它"按这条力—时间曲线推琴键"。
所以 [`research-plan-touch-tone.md`](research-plan-touch-tone.md) 里的实验在 Disklavier 上做不了。
